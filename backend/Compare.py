...
# create a function which compare the time and token count of standard RAG and RAG Fusion
def compare_methods():
    pass
...
@app.post("/compare")
async def compare_methods(question_data: Question):
    original_question = question_data.question
    
    # Initialize metrics dictionary
    metrics = {
        "standard_rag": {"time": None, "tokens": None},
        "rag_fusion": {"time": None, "tokens": None}
    }
    
    # Standard RAG Operation
    start_time = time.time()
    with get_openai_callback() as cb:
        standard_result = global_qa_chain(original_question)
    end_time = time.time()
    
    metrics["standard_rag"]["time"] = end_time - start_time
    metrics["standard_rag"]["tokens"] = cb.total_tokens

    # RAG Fusion Operation
    start_time = time.time()
    queries = generate_queries(original_question, OpenAI())
    all_results = []
    with get_openai_callback() as cb:
        for query in queries:
            docs = global_vectorstore.similarity_search(query)
        ...
        fusion_result = OpenAI()(fusion_prompt.format(context=context, question=original_question))
    end_time = time.time()
    
    metrics["rag_fusion"]["time"] = end_time - start_time
    metrics["rag_fusion"]["tokens"] = cb.total_tokens

    # Aggregate metrics for response
    comparison_response = {
        "comparison": metrics,
        "standard_rag_result": standard_result['result'],
        "rag_fusion_result": fusion_result,  # Placeholder for actual result
    }
